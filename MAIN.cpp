...
inline uint gethsh(char *ch) {
	...
	uint res1=0;
	for(int i=0;i<len;++i) {
		res1=(uint)res1*base1+(uint)ch[i]+(uint)257;
	}
	return res1;
} 
class station{
...
void add_train(){
	...
	for(;;ch=getchar()) {
		...
		if(ch=='i') {
//			char s[21];
			scanf("%s",tmp._trainID);
			hsh=gethsh(tmp._trainID);
			continue;
		}
		if(ch=='n') {
	...
	for(int i=0;i<tmp._stationNum;++i) {int sz=Station_mp.size()+1;Station_mp.insert(std::make_pair(tmp._stations[i].hsh,sz),hsh);}
