...
BPT<std::pair<int,int>,int> mp("User_bpt");
//file operation IO
...
BPT<std::pair<int,int>,int> Order_mp("Order_bpt"),Wait_mp("Wait_bpt");
BPT<std::pair<int,int>,std::pair<int,int>> Station_mp("Station_bpt");
...
BPT<std::pair<int,int>,int> mp_Train("Train_bpt");
...
void add_train(){
	...
	for(int i=0;i<tmp._stationNum;++i) {int sz=Station_mp.size()+1;Station_mp.insert(std::make_pair(tmp._stations[i].hsh.first,sz),hsh);}
...
//	BPT<std::pair<int,int>,std::pair<int,int> >::iterator it1,it2;
	BPT<std::pair<int,int>,std::pair<int,int>>::iterator it1;
	it1=Station_mp.nextData(std::make_pair(tmp1.first,-1));
	//need to modify
	...
	while(1) {
		...
		if(it1.key().first!=tmp1.first) {break;}
		vec.push_back(it1.data());
...
//		debug(2);
	...
	BPT<std::pair<int,int>,std::pair<int,int>>::iterator it1;
	it1=Station_mp.nextData(std::make_pair(S.hsh.first,-1));
	vector<std::pair<answer_to_transfer,int> > ans;
	vector<int> Queue;
	while(1) {
		if(it1.end()) break;
		if(it1.key().first!=S.hsh.first) break;
		vector<int> __v;
		mp_Train.find(it1.data(),__v);
		int traincur=__v[0];
...
//		int dddd=0;
		for(int i=0;i<vec.size();++i) {
			...
			BPT<std::pair<int,int>,std::pair<int,int> >::iterator it2;
			it2=Station_mp.nextData(std::make_pair(mid.hsh.first,-1));
			while(1) {
//				debug(1);
				if(it2.end()) break;
				if(it2.key().first!=mid.hsh.first) break;
				if(it1.data()==it2.data()) {it2++;continue;} 
				vector<int> ___v;
				...
				for(int j=0;j<=totdays;++j) {
					if(std::make_pair(leavemid1,leavemid2)>std::make_pair(arrmid1,arrmid2)) {
						...
						o.arr1=plusdate_2(o.arr1,tmp2.g[r][0]);
						o.arr2=tmp2.tim[r][0];
...
//		if()
		...
		Order_mp.insert(std::make_pair(user_id.first,Mod1-sz),order_write(-1,nw));
	}
	else if(!wishtowait) {
		std::cout<<-1<<"\n";
		return ; 
		...
		Wait_mp.insert(std::make_pair(train_id.first,sz),_pos);
		Order_mp.insert(std::make_pair(user_id.first,Mod1-sz),_pos);
	}
}
void query_order() {
	char ch=getchar();
	...
	BPT<std::pair<int,int>,int>::iterator it,it2;
	it=Order_mp.nextData(std::make_pair(user_id.first,-1));
	int Sum=0;
	it2=it;
	while(1) {
		if(it.end()) break;
		if(it.key().first!=user_id.first) break;
		it++;
		Sum++;
	}
	std::cout<<Sum<<"\n";
//	debug(Sum);
	while(1) {
		if(it2.end()) break;
		if(it2.key().first!=user_id.first) break;
...
inline void Check_waitlist(std::pair<int,int> tmp) {
	...
	BPT<std::pair<int,int>,int>::iterator it; 
	it=Wait_mp.nextData(std::make_pair(tmp.first,-1));
	vector<int> __v;
	mp_Train.find(tmp,__v);
	int traincur=__v[0];
	...
	while(1) {
		if(it.end()) break;
		if(it.key().first!=tmp.first) break;
//		debug(1);
...
void exit() {
	BPT<std::pair<int,int>,int>::iterator it = mp.begin();
	for(;!it.end();it++) {
		int cur=it.data();
		User tmp=User_read(cur);
		tmp._login=0;
