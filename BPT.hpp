...
private:
    ...
    static constexpr int M = 110;
    static constexpr int L = 110;
    ...
    public:
        ...
        void addElement(const Key & _key,int _position,BPT * theTree){
            ...
            for(int i = childSize-1;i > addPos;--i)
                INDEX[i] = INDEX[i - 1];
            for(int i = childSize;i > addPos + 1;--i)
...
public:
    ...
    iterator nextData(const Key & _key){
        if(BasicInformation.root == -1)return begin();
        int flag = 0,pos = -1;
        int leafPosition = findLeaf(_key);
        leafNode * tmpLeafNode = leafDisk.read(leafPosition);
