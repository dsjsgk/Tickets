...
BPT<uint,int,10,50000> mp("User_bpt");
...
BPT<uint,int,10,50000> Order_mp("Order_bpt"),Wait_mp("Wait_bpt");
BPT<uint,int,10,50000> Station_mp("Station_bpt");
...
BPT<uint,int,10,50000> mp_Train("Train_bpt");
		...
		if(ch=='i') {
			...
			for(int i=0;i<_train_._stationNum;++i) {
				...
				if(Station_mp.count(Stationhsh)) {
					...
					Station _temp_=Station_read(Stationcur);
					_temp_.Num++;
...
void query_train() {
	...
	for(;;ch=getchar()) {
		...
		if(ch=='i') {
			...
			Seat _temp;
			if(R.released==1) _temp=Seat_read(R._ticketsSold[dddd]);
...
void query_ticket() {
	...
	vector<int> _V_;
	Station_mp.find(tmp1,_V_);
	if(_V_.size()==0) {
		printf("0\n");
		return ;
	} 
	int Stationcur=_V_[0]; 
	Station Station_S=Station_read(Stationcur);
	vector<uint> vec;
	for(int T1=1;T1<=Station_S.Num;++T1) {
		vec.push_back(Station_S.Train_hsh[T1]);
	}
...
//			dd-=tmp.g[l][1];
			Seat _temp=Seat_read(tmp._ticketsSold[dd]); 
			for(int j=l;j<r;++j) {
...
void query_transfer() {
	...
	Station Station_S=Station_read(Stationcur1);
...
//		if(it1.key().first!=S.hsh) break;
		...
		Seat _temp1=Seat_read(tmp._ticketsSold[dddd]); 
...
//				if(it2.key().first!=mid.hsh) break;
				if(Station_S.Train_hsh[T1]==Station_mid.Train_hsh[T2]) {continue;} 
				vector<int> ___v;
				mp_Train.find(Station_mid.Train_hsh[T2],___v);
...
//					int tot=0;
				...
				//////////////////////////////////////////////
					...
					Seat _temp2=Seat_read(tmp2._ticketsSold[___d]); 
					for(int k=l;k<r;++k) {
...
//	debug(tt);	
	long long  totNum=0;
	int ok=1;
	Seat _temp=Seat_read(tmp._ticketsSold[tt]); 
	for(int i=l;i<r;++i) {
...
//		if()
		...
		Order_mp.insert(user_id,order_write(-1,nw));
	}
		...
		Wait_mp.insert(train_id,_pos);
		Order_mp.insert(user_id,_pos);
	}
}
void query_order() {
	char ch=getchar();
...
//	BPT<std::pair<uint,int>,int>::iterator it,it2;
	vector<int> _Order; 
	Order_mp.find(user_id,_Order);
	std::cout<<_Order.size()<<"\n";
	int SZ=_Order.size();
	for(int i=0;i<SZ;++i) {
		order tmp=order_read(_Order[i]);
...
//		debug(i);
		order nw=order_read(_Order[i]);
		
		if(nw.status!=0) {continue;}
		int ok=1;
		int dd=get_date(o._st,nw.date);
		Seat _temp=Seat_read(o._ticketsSold[dd]); 
...
void exit() {
	BPT<uint,int,10,50000>::iterator it = mp.begin();
	for(;!it.end();it++) {
		int cur=it.data();
		User tmp=User_read(cur);
		tmp._login=0;
