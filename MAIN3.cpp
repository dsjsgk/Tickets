...
BPT<std::pair<uint,int>,uint> Station_mp("Station_bpt");
...
void release_train() {
	...
	for(;;ch=getchar()) {
		...
		if(ch=='i') {
			...
			for(int i=0;i<_train_._stationNum;++i) {int sz=Station_mp.size()+1;Station_mp.insert(std::make_pair(_train_._stations[i].hsh,sz),tmp);}
...
//		if(tmp.released!=1) {it1++;continue;}
		...
		for(int i=0;i<vec.size();++i) {
			...
			BPT<std::pair<uint,int>,uint>::iterator it2;
			it2=Station_mp.nextData(std::make_pair(mid.hsh,-1));
			while(1) {
				if(it2.end()) break;
